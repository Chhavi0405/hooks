...
export default function Home() {
  ...
  const { getDate,getMMddyyyy,getddMMyy,getDay,getMonth,getYear,getTimeInHHMM,getTimeInHHMMSS,getyyyyMMdd } = useFormat();
...
setIsTime(getTimeInHHMM(getDate()))
  ...
  return (
   ...
   <br/>
   <button onClick={()=>getddMMyy(getDate())}>format Date2 in ddMMyy</button>
   <br/>
   <button onClick={()=>getyyyyMMdd(getDate())}>format Date3 in yyyyMMdd</button>
   <br/>
  ...
  {isTime}
  <br/>
  <button onClick={()=>getTimeInHHMMSS(getDate())}>time in HH:MM:SS</button>
    </>
  )
}
